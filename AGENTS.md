...
- PDF generation is **PDFsharp + MigraDoc 6.2.4** (MIT), not QuestPDF/PdfSharpCore. Both were replaced in the same change: QuestPDF's Community license doesn't cover EDEEste (public-sector entities and orgs over its revenue threshold aren't eligible, regardless of size), and PdfSharpCore dragged in `SixLabors.ImageSharp 1.0.4` (known CVEs) as a transitive dependency. Don't reintroduce either package without a licensing decision — see the migration plan/memory for the full analysis.
- **`PageSetup.PageWidth`/`PageHeight` are 0 if read before the document is rendered** — MigraDoc only computes them during `PdfDocumentRenderer.RenderDocument()`, not when `PageFormat` is assigned. Reading them earlier to size a table column or an image silently produces negative widths, which collapses the whole table into an unreadable sliver (found by rasterizing a generated PDF to PNG and comparing — the bug wasn't obvious from a build or a page count, only from actually looking at the render). Use the static `PageSetup.GetPageSize(PageFormat.A4, out ancho, out alto)` instead; see `PdfConsolidadorService.ObtenerAreaDeContenido()`.
- Fonts are **embedded**, not read from the OS: `ResolutorFuentesEmbebidas` (`IFontResolver`) serves Lato (SIL OFL, same face QuestPDF used by default) regardless of the family name requested, so the same PDF renders identically under IIS, where `GlobalFontSettings.UseWindowsFontsUnderWindows` isn't reliable (the app pool identity may not have access to the Windows fonts folder) — and so any future Linux/container deployment doesn't need a font install step either. Registered once via `ResolutorFuentesEmbebidas.Registrar()` in `AddInfrastructure`; PDFsharp throws if `GlobalFontSettings.FontResolver` is reassigned after first use, so this can't be called per-request or per-document.
- Save with `documento.Save(stream, closeStream: false)`, not `Save(stream)` (which defaults to closing it) — if the stream is read again afterward (e.g. `MemoryStream.ToArray()`), a default `Save` leaves it disposed first. `PdfDocumentOpenMode.ReadOnly` is obsolete in PDFsharp 6.x; use `Import`.
- Page numbers in the consolidated PDF are stamped **after** merging, with `XGraphics.FromPdfPage(..., XGraphicsPdfPageOptions.Append)`. Neither QuestPDF's nor MigraDoc's own footer can do it: the expediente is assembled from several independent PDFs plus the comprobantes' own files, so each piece would restart at 1.
- A comprobante whose magic bytes match but whose file is truncated or corrupted (e.g. an upload cut off by a network drop) doesn't fail to save — `ValidadorComprobante` rejects anything under 512 bytes even with a valid signature (a real PDF/PNG/JPEG never gets close), and `PdfConsolidadorService.AgregarComprobante` also wraps the merge of an already-saved comprobante in a try/catch, inserting a warning page instead of throwing. Both exist because `ComprobanteAdjunto` is HMAC-signed and immutable once saved — the custodio has no way to re-upload, so a comprobante that can't be parsed must never block the whole reposición.
- Runtime-uploaded and generated files live under `App_Data/uploads` (root resolved from `IHostEnvironment.ContentRootPath` via `OpcionesAlmacenamiento`, not `Directory.GetCurrentDirectory()` — that call isn't reliable under IIS). They are **not** under `wwwroot` and **not** served by `MapStaticAssets` (it uses a build-time manifest anyway). Serve them through an endpoint — see `Presentation/Endpoints/GastoEndpoints.cs`, `ReposicionEndpoints.cs`, and `PerfilEndpoints.cs`.
...
- Render modes of the account pages are deliberate. `Account/Manage` (Perfil) is interactive, so moving between Inicio and Perfil doesn't force a full reload and a new circuit. `Manage/ChangePassword`, Login, Register and the whole recovery flow each carry their own `[ExcludeFromInteractiveRouting]` and stay static SSR **on purpose**: no password ever travels over the circuit, only in a normal HTTP POST (decided 2026-09-08, reaffirmed 2026-09-11 — don't make ChangePassword interactive to get rid of the flash on that tab). The profile photo is still posted by a plain HTML form even though Perfil is interactive: never switch it to `InputFile` + `OpenReadStream()`, which would put the bytes back on the circuit. To write the user's own row from the circuit (e.g. the extension in Perfil), use a fresh DI scope, hand it the circuit's authentication state (`IHostEnvironmentAuthenticationStateProvider.SetAuthenticationState`, or the audit log says "Sistema"), load a fresh copy and call `UserManager.UpdateAsync`. Not `SetPhoneNumberAsync` (it rotates the security stamp, so the session closes itself at the next revalidation) and not `ExecuteUpdateAsync` (it skips the audit log). `UpdateAsync` on an old or circuit-tracked copy is also wrong: `UserStore` does `Attach` + `Update`, so it rewrites the whole row with stale values or throws on `Attach`.
...
- Interceptors (`AuditoriaInterceptor`, `IntegridadInterceptor`) are registered as `services.AddSingleton<IInterceptor, ...>()` and passed to `options.AddInterceptors(serviceProvider.GetServices<IInterceptor>())` inside the `AddDbContext` lambda. EF keys its internal service-provider cache on the interceptor list's *instances* — anything less than a stable Singleton (Scoped was tried two different ways, both failed the same way) builds a fresh provider per request and throws `ManyServiceProvidersCreatedWarning` after ~20 requests. Because they're Singleton, `AuditoriaInterceptor` can't take the Scoped `ICurrentUserService` by constructor; it asks the `ApplicationDbContext` being saved instead (`eventData.Context` → `ObtenerUsuarioAuditoriaAsync`), which is Scoped and receives `ICurrentUserService` by constructor — same DI scope as the request or circuit doing the save. Don't move interceptors back to Scoped and don't inject `ICurrentUserService` into them directly. Don't carry the user id in an `AsyncLocal` assigned inside an `async` method either (the previous design): an async method restores the ExecutionContext on exit, so the value never reaches the caller and every audit row ended up as "Sistema". A save made from a scope created by hand with `IServiceScopeFactory` inside a circuit has an uninitialized `AuthenticationStateProvider` and can also end up as "Sistema" — hand that scope the circuit's authentication state before saving.
